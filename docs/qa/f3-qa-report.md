...
| 5 | Refresh responde; app vivo | Botão 1 → heartbeat `updatedAt` **16:17:34Z → 16:17:35Z**, processo vivo | ✅ PASS |
| 6 | Quit pelo painel → sem zumbi | Botão 4 → `pgrep` **vazio** | ✅ PASS |
...
- **OBS-2 — painel fecha quando a janela Analytics ganha foco:** MenuBarExtra `.window` perde o key status; comportamento esperado do macOS, sem impacto (cada janela tem função própria).
