...
| 2 | **Janela Add Account** (abre, overlap BLOQUEIA, registra irmão legítimo) | Comportamento live: E2E v4 §10 — conta registrada programaticamente entra no ciclo com soma exata (`7796399 == 6490348 + 333 + 1305718`, sem duplicação), toggle/remove e frota 30 idem. Validação do form pinada em unidade: vazio bloqueia; inexistente avisa; **overlap bloqueia** (`"Directory overlaps an existing scan root."`); não-regular avisa (Red Team caso 4). Render das linhas de conta com badge **`invalid path`** (vermelho): `f4-account-rows.png`. Nota: o RENDER da janela do form não compôs no ImageRenderer (Form) e a janela real não era AX-enumerável nesta sessão — estado visual do form coberto pelas strings pinadas de validação | ✅ PASS |
| 3 | **Toggle/remove conta** | Unidade: `inactiveAccountLeavesCycle` (toggle tira do agregado e do ciclo; reativa retoma), remoção idempotente + prune de caches por conta (`thirtyAccountsAllCycleInOnePass` removal batch). Live: E2E §10 — toggle OFF soma exata sem a parte da conta (`6490792 == 6490681-SC2+111`... esperado canônico+append ✓) e remoção em lote devolve ao F2; registro permanece com `active=0` | ✅ PASS |
...
| 5 | **Orçamento de recursos** | E2E v4: app ocioso 60 s (DB aberto) **17 MB / 0.0%**; pós-migração idem; **com 31 contas: 16.9–17.3 MB ≤ 40 MB, cpu 0.0%**; RT7 (ticker 10 min, painel aberto): footprint estável 25.3→25.2 MB (log commitado) | ✅ PASS |
...
- §10 multi-conta (18): registro programático → ciclo cobre as duas (soma exata), eventos/cursor POR CONTA no DB, toggle/remove exatos, frota de 30 num ciclo só (4–5 s), higiene do payload (798 B, sem paths/ids), orçamento com 31 contas, remoção → layout F2.
